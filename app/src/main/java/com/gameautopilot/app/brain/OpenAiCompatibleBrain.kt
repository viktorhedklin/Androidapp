...
YOU MUST RESPOND WITH STRICT JSON of this exact shape and nothing else:
{
  ...
  "actions": [
    ...
    or {"type":"longPress","x":<int>,"y":<int>,"durationMs":<int>}
    or {"type":"wait","ms":<int>}
    or {"type":"back"}
    or {"type":"noop"}
  ],
...
RULES:
- Coordinates are absolute pixels. Stay inside [0, ${ctx.screenWidth}) x [0, ${ctx.screenHeight}).
...
""".trimIndent()
    ...
    private fun buildUserText(ctx: BrainContext): String {
        val ocr = ctx.ocrLines.take(60).joinToString("\n").ifBlank { "(none)" }
        val a11y = ctx.a11yLines.take(80).joinToString("\n").ifBlank { "(none)" }
